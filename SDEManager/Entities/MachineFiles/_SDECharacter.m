...
// Make changes to SDECharacter.m instead.

#import "_SDECharacter.h"

const struct SDECharacterAttributes SDECharacterAttributes = {
...
const struct SDECharacterRelationships SDECharacterRelationships = {
	.abilities = @"abilities",
	.actions = @"actions",
	.items = @"items",
...
const struct SDECharacterFetchedProperties SDECharacterFetchedProperties = {
};

@implementation SDECharacterID
@end

@implementation _SDECharacter

+ (id)insertInManagedObjectContext:(NSManagedObjectContext*)moc_ {
	NSParameterAssert(moc_);
	return [NSEntityDescription insertNewObjectForEntityForName:@"SDECharacter" inManagedObjectContext:moc_];
}
...
+ (NSEntityDescription*)entityInManagedObjectContext:(NSManagedObjectContext*)moc_ {
	NSParameterAssert(moc_);
	return [NSEntityDescription entityForName:@"SDECharacter" inManagedObjectContext:moc_];
}

- (SDECharacterID*)objectID {
	return (SDECharacterID*)[super objectID];
}
...
+ (NSSet*)keyPathsForValuesAffectingValueForKey:(NSString*)key {
	...
	if ([key isEqualToString:@"isShapeshiftedValue"]) {
		NSSet *affectingKey = [NSSet setWithObject:@"isShapeshifted"];
		keyPaths = [keyPaths setByAddingObjectsFromSet:affectingKey];
		return keyPaths;
	}
...
@dynamic isShapeshifted;



- (BOOL)isShapeshiftedValue {
	NSNumber *result = [self isShapeshifted];
	return [result boolValue];
}

- (void)setIsShapeshiftedValue:(BOOL)value_ {
	[self setIsShapeshifted:[NSNumber numberWithBool:value_]];
}

- (BOOL)primitiveIsShapeshiftedValue {
	NSNumber *result = [self primitiveIsShapeshifted];
	return [result boolValue];
}

- (void)setPrimitiveIsShapeshiftedValue:(BOOL)value_ {
	[self setPrimitiveIsShapeshifted:[NSNumber numberWithBool:value_]];
}
